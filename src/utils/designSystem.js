...
const ui = {
  ...
  isSystemAppearance: Appearance.getColorScheme() === 'dark',
};

const colors = {
  primary: '#D7FDF7', // cyan
  secondary: '#5383b8', // blue
  accent: '#2ab4f9', // blue
  success: '#84bf0f',
  warning: '#ffbe0c',
  danger: '#ff5842',
...
export const getThemeStatusBarStyle = ca => {
  const current = ca ?? {
    value: ui.appearance,
...
export const getNavigationTheme = ca => {
  const current = ca ?? {
    value: ui.appearance,
    system: ui.isSystemAppearance,
  };
...
export const getHeaderBlurEffect = ca => {
  const current = ca ?? {
    value: ui.appearance,
    system: ui.isSystemAppearance,
  };
